...
function buildKillRow(kill) {
  ...
  div.className = `kill-row justify-between ${isWhale ? 'whale' : ''}`
  div.dataset.alliance = (kill.allianceName || '').toLowerCase()

  div.innerHTML = `
    <div class="flex items-center" style="flex:1;">
      <a href="${escapeHtml(kill.zkillUrl)}" target="_blank" rel="noopener" class="kill-info-link flex items-center">
        <div class="ship-icon-container">
          <img src="${escapeHtml(kill.shipImageUrl)}" alt="Ship Render" class="w-full h-full object-cover" loading="lazy" />
        </div>
        <div class="kill-info ml-2">
          <div>
    ...
    <div class="flex items-center">
      <div class="corp-square-container mr-3">
      ...
      <div class="text-end" style="width:100px;">
        <div class="${iskClass} font-bold isk-value"></div>
      </div>
    </div>
  `
...
function bindSocketHandlers() {
  ...
  socket.on('player-count', (data) => {
    ...
    headerPlayers.innerText = (data && data.active)
